...
let server
exports.startServer = function (port = 3876) {
            ...
            socket.on('data', function (msg) {
                ...
                try {
                    ...
                    switch (msg.cmd) {
                        ...
                        case 'deposit':
                        case 'withdraw':
                            response = logTransaction(msg.cmd, msg.amount, msg.id)
                            break
                        case 'register':
                            response = register(msg.id, msg.publickey)
                            break
                        default:
    ...
    function balance (id) {
        let lastTransaction = last(transactions.filter(t => t.value.id === id))
        let hash = lastTransaction && lastTransaction.hash || ''
        return {
            cmd: 'balance',
    ...
    function readTransactions () {
        try {
            ...
            log('Creating empty transactions.')
            return []
        }
    }
